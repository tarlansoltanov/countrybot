...
from countrybot.utils.population import total_population_graph, men_population_graph, women_population_graph
...
async def men_population(call: CallbackQuery, bot: AsyncTeleBot):
    async with bot.retrieve_data(call.from_user.id) as data:
        country = data['country']
    
    logging.info(f'User {call.from_user.id} selected showing men population graph of the country : {country["name"]["common"]}')

    await bot.delete_message(call.from_user.id, call.message.message_id)
    await bot.send_photo(call.from_user.id, open(men_population_graph(country['cca3']), 'rb'), f'Men Population Graph Of {country["name"]["common"]}', reply_markup=back_keyboard(1))


async def women_population(call: CallbackQuery, bot: AsyncTeleBot):
    async with bot.retrieve_data(call.from_user.id) as data:
        country = data['country']
    
    logging.info(f'User {call.from_user.id} selected showing women population graph of the country : {country["name"]["common"]}')

    await bot.delete_message(call.from_user.id, call.message.message_id)
    await bot.send_photo(call.from_user.id, open(women_population_graph(country['cca3']), 'rb'), f'Women Population Graph Of {country["name"]["common"]}', reply_markup=back_keyboard(1))